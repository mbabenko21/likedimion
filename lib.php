...
function base_params($player)
{
    $player[base_params] = unserialize($player[base_params]);
    return $player;
}

function fact_params($player)
{
...
function display($page, $title, $style)
{
    ...
    $header_block = '<?xml version="1.0" encoding="utf-8"?>
		...
		<head>
		<title>' . $title . '</title>
		<link rel="stylesheet" media="all" type="text/css" href="./css/standart.css" />
</head>
<body>' . "<div class='f'>";
    $foot_block = '</div></body></html>';
    $rnd = rand(1, 100);
    $t = "";
...
{
    $page .= "<br/><b>$item[name]</b><br/>
		<br/>$item[info]<br/>";
    $about_item = $item[about_item];
    if ($item[type] == "weapon") {
        $page .= "<br/>Требуемый уровень: $about_item[req_level]";
        if ($about_item[type_weap] == "fire" and $about_item[maxpatrons] > 0) {
            $page .= "<br/>Калибр: $about_item[calibr]
             <br/>Обойма: $about_item[patrons]/$about_item[maxpatrons]";
            if (!empty($about_item[idpatrons]) and $about_item[patrons] > 0) {
                ...
                $page .= "<br/>Заряжено: $patronname";
            }
            if ($about_item[arrayfire] > 0) {
                $page .= "<br/>Очередь: $about_item[arrayfire] патрон";
            }
        } elseif ($about_item[type_weap] == "throw") {
            $page .= "<br/>Тип: Метательное";
        }
        elseif ($about_item[type_weap] == "mellee") {
            $page .= "<br/>Тип: ближнего боя";
        }
        if ($about_item[type_dmg] == "normal") {
            $type = "Нормальный";
        } elseif ($about_item[type_dmg] == "plazma") {
            $type = "Плазма";
        }
        elseif ($about_item[type_dmg] == "boom") {
            $type = "Взрыв";
        }
        elseif ($about_item[type_dmg] == "volt") {
            $type = "Электричество";
        }
        $page .= "<br/>Тип урона: $type
           <br/>Урон: $about_item[mindmg]-$about_item[maxdmg]";
        if ($about_item[odbonus] != 0) {
            $page .= "<br/>Од на выстрел: " . (3 + $about_item[odbonus]);
        }
        if ($about_item[sniperbonus] != 0) {
            $page .= "<br/>Дополнительный шанс попасть: $about_item[sniperbonus]%";
        }
        if ($about_item[crash] != 0) {
            $page .= "<br/>Шанс осечки: $about_item[crash]%";
        }

    } elseif ($item[type] == "bodyarm") {
        $page .= "<br/>Требуемый уровень: $about_item[req_level]
            <br/><b>Сопротивления:</b>";
        if ($about_item[resnormal] > 0) {
            $page .= "<br/>Нормальному урону: $about_item[resnormal]%";
        }
        if ($about_item[resplazma] > 0) {
            $page .= "<br/>Плазме: $about_item[resplazma]%";
        }
        if ($about_item[resboom] > 0) {
            $page .= "<br/>Взрывам: $about_item[resboom]%";
        }
        if ($about_item[resvolt] > 0) {
            $page .= "<br/>Электричеству: $about_item[resvolt]%";
        }
        if ($about_item[respoison] > 0) {
            $page .= "<br/>Отравлению: $about_item[respoison]%";
        }
        if ($about_item[resrad] > 0) {
            $page .= "<br/>Радиации: $about_item[resrad]%";
        }
        if ($about_item[bonusdex] > 0) {
            $page .= "<br/>Доп. шанс уворота: $about_item[bonusdex]%";
        }
        if ($about_item[kb] > 0) {
            $page .= "<br/>Коэффициент брони: $about_item[kb]";
        }
    }
    elseif ($item[type] == "patron") {
        $page .= "<br/>Калибр: $about_item[calibr]";
        if ($about_item[moddmg] != 0) {
            $page .= "<br/>Плюс к урону: $about_item[moddmg]";
        }
    }
    elseif ($item[type] == "medicament") {
        if ($about_item[param] == "hungry") {
            if ($about_item[type] == "const") {
                $page .= "<br/>Уталяет голод на $about_item[value] единиц";
            } elseif ($about_item[type] == "procent") {
                $page .= "<br/>Уталяет голод на $about_item[value]%";
            }
        }
        if ($about_item[param] == "rad") {
            if ($about_item[type] == "const") {
                $page .= "<br/>Выводит $about_item[value] единиц радиации из организма";
            } elseif ($about_item[type] == "procent") {
                $page .= "<br/>Выводит $about_item[value]% радиации из организма";
            }
        }
        if ($about_item[param] == "poison") {
            if ($about_item[type] == "const") {
                $page .= "<br/>Выводит $about_item[value] единиц ядов из организма";
            } elseif ($about_item[type] == "procent") {
                $page .= "<br/>Выводит $about_item[value]% ядов из организма";
            }
        } elseif ($about_item[param] == "hit_points") {
            if ($about_item[type] == "const") {
                $page .= "<br/>Повышает здоровье на $about_item[value] единиц";
            } elseif ($about_item[type] == "procent") {
                $page .= "<br/>Повышает здоровье на $about_item[value]%";
            }
            elseif ($about_item[type] == "full") {
                $page .= "<br/>Здоровье становится больше максимума на $about_item[value]%";
            }
        }
    ...
    $page .= "<br/>Базовая цена: $about_item[cena]";
    if ($about_item[massa] != 0) {
        $page .= "<br/>Вес: $about_item[massa]<br/>";
    }
    return $page;
}
...
{
    $sql = mysql_query("SELECT char_name,level,exp,needexp,hit_points,maxhp,hungry_points,od," .
        "maxod,crit_chance,fact_params,fact_resists,trauma,weapon1,weapon2,bodyarm,money,onlinetime FROM users WHERE id='$id' LIMIT 1");
    ...
    if ($player[onlinetime] > (time() - 5 * 60)) {
        $online = "<br/>Онлайн";
    } else {
        $online = "<br/>Последний раз был " . date("j.m.", $player[onlinetime]) . (date("Y", $player[onlinetime]) + 170) . date("  G:i", $player[onlinetime]);
    }
    $page = "<br/><br/><b>$player[char_name]</b>$online<br/>

                <br/>Деньги $player[money]
                <br/>Оружие 1: $weapon1[name]
                <br/>Оружие 2: $weapon2[name]
                <br/>Броня:  $bodyarm[name]<br/>
    			<br/>Уровень $player[level]
    			<br/>Текущий опыт $player[exp]
    			<br/>Опыт для следущего уровня $player[needexp]";
    ...
    $page .= "<br/><a href='./?view=player&amp;stat=$id'>Статистика</a>
    ...
    $page .= "<br/><br/><b>Сопротивления</b><br/>
    ...
    if ($trauma[lefthand] == "on") {
        $page .= "<br/><b>Сломана левая рука</b><br/>";
    }
    if ($trauma[righthand] == "on") {
        $page .= "<br/><b>Сломана правая рука</b><br/>";
    }
    if ($trauma[leftleg] == "on") {
        $page .= "<br/><b>Сломана левая нога</b><br/>";
    }
    if ($trauma[rightleg] == "on") {
        $page .= "<br/><b>Сломана правая нога</b><br/>";
    }
    if ($trauma[eye] == "on") {
        $page .= "<br/><b>Поврежден глаз</b><br/>";
    }
    $page .= "<br/><a href='./?do=mail&amp;mod=write&amp;to=$player[char_name]'>Написать письмо</a>";
    return $page;
...
function add_to_garbage($loc_id, $item) //добавление вещей к мусору
{
    ...
    $on_enter = "if (time()>" . (time() + 10 * 60) . ") {destroy_garbage($loc_id);}";
    ...
    if ($c != 1) { //создаем новый мусор)))
        $bag[] = $item;
        $obj = array("name" => "Всякий хлам", "info" => "Под ногами валяется всякий хлам", "type" => "garbage",
...
function add_to_inv($itemid, $colvo, $bag, $userid, $usergruz) //Добавление шмота в инвентарь игрока
{
    ...
    if (mysql_num_rows($sql) != 1) {
        $page .= "Предмета с ID $_POST[itemid] не существует!";
    } else {
        $item = mysql_fetch_array($sql);
        $item[colvo] = $colvo;
        $item[about_item] = unserialize($item[about_item]);
        ...
        if (!isset($k) and $colvo > 0) {
            $k = sizeof($bag);
            $bag[] = $item;
        }

        if ($bag[$k][colvo] <= 0) {
            $bag = delete_element($bag, $k);
...
function have_item($bag, $itemid)
{ //возвращает количество вещей в инвентаре
...
function add_user($user, $temp)
{
    if (isset($temp[base_params])) {
        if (!is_array($user[base_params])) {
            $user[base_params] = unserialize($user[base_params]);
        }
        foreach ($temp[base_params] as $key => $value) {
            if ($key == "life") {
                $temp[maxhp] = $temp[maxhp] + $value * 2 * (4 + intval(ceil($user[base_params][endur] / 2)));
            }
    ...
    if (isset($temp[bag])) {
        if (!is_array($user[bag])) {
            $user[bag] = unserialize($user[bag]);
        }
    ...
    if (isset($temp[quests])) {
        if (!is_array($user[quests])) {
            $user[quests] = unserialize($user[quests]);
        }
    ...
    if (isset($temp[recepts])) {
        if (!is_array($user[recepts])) {
            $user[recepts] = unserialize($user[recepts]);
        }
